...
#
if [ -f $config_file ]; then # 컨피그 파일이 있는지
    # 존재한다면 그냥 패스
    echo
...
exec 0<$config_file # 컨피그 파일로 Standard Input으로 redirect(0 = STDIN)
#
read file_name # Input(컨피그 파일)의 첫줄 file_name으로 읽기(exec, read 함께 쓰임)
#
while [ $? -eq 0 ]; do # 백업할 파일 리스트업
    # -o or
    if [ -f $file_name -o -d $file_name ]; then # 파일이나 디렉토리가 있는지 확인
